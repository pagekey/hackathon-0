...
async function getGithubContributions(username: string): Promise<Contribution[]> {
  try {
    ...
    if (response.status == 200) {
      ...
      const result = contributions.map(async (contribution) => {
        const date = contribution.getAttribute('data-date');
        const level = parseInt(contribution.getAttribute('data-level'));
      ...
      const filteredResponses = responses.filter((contribution) => contribution.count > 0);
      console.log('got big result', filteredResponses);
